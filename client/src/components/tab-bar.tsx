...
export default function TabBar({ onPlusClick, onShopClick }: TabBarProps) {
  ...
  const tabs = [
    ...
    { 
      ...
      path: '/balance',
      icon: (active: boolean) => (
        <svg className={`w-5 h-5 transition-colors duration-300 ${active ? 'text-blue-600' : 'text-gray-400 dark:text-gray-500'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
