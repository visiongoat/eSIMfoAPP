...
export default function HomeScreen() {
  ...
  // Dual API geolocation detection with better error handling
  const detectUserLocation = async () => {
    setLocationStatus('loading');
    
    // First try: ipapi.co with timeout and error handling
    try {
      ...
      console.log('⚠️ ipapi.co failed:', error instanceof Error ? error.message : 'Unknown error');
    }

    // Second try: ip-api.com with timeout and error handling
    try {
      ...
      console.log('⚠️ ip-api.com failed:', error instanceof Error ? error.message : 'Unknown error');
    }
  ...
  if (showFullScreenSearch) {
    return (
      <div className="mobile-screen bg-blue-50/30 dark:bg-gray-900 min-h-screen animate-search-expand">
        {/* Search Header */}
                ...
                {searchResults.localCountry && (
                  <div>
                    ...
                    <button
                      ...
                      className="w-full px-3 py-1.5 bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-700 border border-gray-200 dark:border-gray-700 rounded-md text-left transition-all duration-200 flex items-center justify-between"
                    >
                      <div className="flex items-center space-x-4">
                        <div className="w-8 h-5 overflow-hidden flex items-center justify-center bg-gray-700 border border-gray-300 rounded-[2px] sim-card-flag">
                        ...
                        <div>
                          <div className="text-gray-900 dark:text-white font-medium text-sm">{searchResults.localCountry.name}</div>
                  ...
                  <div>
                    ...
                    {searchResults.regionalPackages.length > 3 && (
                      <button
                        ...
                        className="w-full mt-3 px-3 py-2 bg-gray-200 dark:bg-gray-700 hover:bg-gray-300 dark:hover:bg-gray-600 rounded-lg text-left transition-all duration-200 flex items-center justify-between group"
                      >
                        <div className="flex items-center space-x-2.5">
                          <div className="w-7 h-7 bg-green-500/30 rounded-full flex items-center justify-center">
                    ...
                    <div className="space-y-2">
                      {recentSearches.map((search, index) => (
                        <button
                          ...
                          className="w-full px-3 py-1.5 bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-700 border border-gray-200 dark:border-gray-700 rounded-md text-left transition-all duration-200 flex items-center justify-between group"
                        >
                          <div className="flex items-center space-x-4">
                            {(() => {
                              const matchingCountry = countries.find(c => c.name.toLowerCase() === search.toLowerCase());
                            ...
                            <div>
                              <div className="text-gray-900 dark:text-white font-medium text-sm">{search}</div>
      ...
      <div className="relative sticky top-0 z-10 py-4">
        <div className="max-w-screen-md mx-auto px-4">
          ...
          <div className="flex items-center justify-end flex-1 ml-4">
            {/* Offline Indicator */}
            {!isOnline && (
              <div className="flex items-center bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 px-2 py-1 rounded-md mr-3">
            ...
            <button 
              ...
              className="flex flex-col items-end bg-gradient-to-br from-green-50/80 via-white to-emerald-50/80 dark:from-green-900/10 dark:via-gray-800 dark:to-emerald-900/10 border border-green-200/60 dark:border-green-700/40 px-3 py-2 rounded-xl shadow-sm hover:shadow-md backdrop-blur-sm transition-all duration-200 active:scale-95"
              data-testid="button-balance"
            >
              <span className="text-xs text-green-600 dark:text-green-400 font-medium opacity-80">
                fo balance
              </span>
...
[
          ...
          <div className="space-y-3">
            ...
            <div className="continent-card continent-americas rounded-xl p-4 shadow-sm animate-stagger-fade stagger-delay-2 touch-feedback cursor-pointer">
              <div className="flex items-center justify-between relative z-10">
                <div className="flex items-center space-x-3">
                  <div className="continent-icon w-12 h-12 bg-orange-200 dark:bg-orange-700 rounded-xl flex items-center justify-center">
                    <img 
                      src={americasIcon} 
                      alt="Americas"
                      className="w-10 h-10 object-contain"
            ...
            <div className="continent-card continent-middle-east rounded-xl p-4 shadow-sm animate-stagger-fade stagger-delay-4 touch-feedback cursor-pointer">
              <div className="flex items-center justify-between relative z-10">
                <div className="flex items-center space-x-3">
                  <div className="continent-icon w-12 h-12 bg-purple-200 dark:bg-purple-700 rounded-xl flex items-center justify-center">
                    <img 
                      src={middleEastIcon} 
                      alt="Middle East"
                      className="w-10 h-10 object-contain"
        ...
        <div className="mt-6 bg-gradient-to-br from-blue-50/80 via-white to-indigo-50/60 dark:from-gray-800 dark:via-gray-800 dark:to-blue-900/20 rounded-xl sm:rounded-2xl border border-blue-100/50 dark:border-gray-700 p-3 sm:p-4 relative">
          ...
          <div className="mb-4 pb-4 -mx-3 sm:-mx-4 -mt-3 sm:-mt-4 px-3 sm:px-4 pt-3 sm:pt-4 bg-gradient-to-r from-blue-100/60 via-indigo-50/40 to-purple-100/50 dark:from-blue-900/30 dark:via-indigo-900/20 dark:to-purple-900/30 border-b border-blue-100/50 dark:border-gray-700 relative">
            ...
            {/* Single Line App Icons - Compact CTR booster */}
            <div className="flex items-center space-x-3">
              <span className="text-[11px] text-gray-600 dark:text-gray-400">Works with</span>
              <div className="flex items-center space-x-2">
                <svg className="w-4 h-4 text-gray-600 dark:text-gray-400" viewBox="0 0 24 24" fill="currentColor"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg>
                <svg className="w-4 h-4 text-gray-600 dark:text-gray-400" viewBox="0 0 24 24" fill="currentColor"><path d="M11.944 0A12 12 0 0 0 0 12a12 12 0 0 0 12 12 12 12 0 0 0 12-12A12 12 0 0 0 12 0a12 12 0 0 0-.056 0zm4.962 7.224c.1-.002.321.023.465.14a.506.506 0 0 1 .171.325c.016.093.036.306.02.472-.18 1.898-.962 6.502-1.36 8.627-.168.9-.499 1.201-.82 1.23-.696.065-1.225-.46-1.9-.902-1.056-.693-1.653-1.124-2.678-1.8-1.185-.78-.417-1.21.258-1.91.177-.184 3.247-2.977 3.307-3.23.007-.032.014-.15-.056-.212s-.174-.041-.249-.024c-.106.024-1.793 1.14-5.061 3.345-.48.33-.913.49-1.302.48-.428-.008-1.252-.241-1.865-.44-.752-.245-1.349-.374-1.297-.789.027-.216.325-.437.893-.663 3.498-1.524 5.83-2.529 6.998-3.014 3.332-1.386 4.025-1.627 4.476-1.635z"/></svg>
                <svg className="w-4 h-4 text-gray-600 dark:text-gray-400" viewBox="0 0 24 24" fill="currentColor"><path d="M12.001 18.42c-.263 0-.498-.104-.71-.296-.534-.482-1.26-1.209-1.902-2.263-.679-1.12-1.49-2.778-1.49-4.75 0-1.61.56-2.716 1.241-3.39.667-.661 1.502-.99 2.316-1.006h.09c.815.015 1.65.345 2.317 1.005.682.675 1.241 1.78 1.241 3.39 0 1.973-.811 3.632-1.49 4.751-.642 1.054-1.368 1.781-1.902 2.263-.212.192-.448.296-.711.296zm0-9.765c-.466.001-.89.19-1.207.504-.405.401-.712 1.105-.712 2.073 0 1.423.576 2.73 1.16 3.69.39.642.78 1.122 1.067 1.426.104.11.194.199.269.267.075-.068.165-.158.269-.267.287-.304.677-.784 1.067-1.426.584-.96 1.16-2.267 1.16-3.69 0-.968-.307-1.672-.712-2.073-.317-.314-.741-.503-1.207-.504h-.154zM12 0C5.373 0 0 5.373 0 12s5.373 12 12 12 12-5.373 12-12S18.627 0 12 0zm0 22.5C6.21 22.5 1.5 17.79 1.5 12S6.21 1.5 12 1.5 22.5 6.21 22.5 12 17.79 22.5 12 22.5z"/></svg>
                <svg className="w-4 h-4 text-gray-600 dark:text-gray-400" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z"/></svg>
                <svg className="w-4 h-4 text-gray-600 dark:text-gray-400" viewBox="0 0 24 24" fill="currentColor"><path d="M12.525.02c1.31-.02 2.61-.01 3.91-.02.08 1.53.63 3.09 1.75 4.17 1.12 1.11 2.7 1.62 4.24 1.79v4.03c-1.44-.05-2.89-.35-4.2-.97-.57-.26-1.1-.59-1.62-.93-.01 2.92.01 5.84-.02 8.75-.08 1.4-.54 2.79-1.35 3.94-1.31 1.92-3.58 3.17-5.91 3.21-1.43.08-2.86-.31-4.08-1.03-2.02-1.19-3.44-3.37-3.65-5.71-.02-.5-.03-1-.01-1.49.18-1.9 1.12-3.72 2.58-4.96 1.66-1.44 3.98-2.13 6.15-1.72.02 1.48-.04 2.96-.04 4.44-.99-.32-2.15-.23-3.02.37-.63.41-1.11 1.04-1.36 1.75-.21.51-.15 1.07-.14 1.61.24 1.64 1.82 3.02 3.5 2.87 1.12-.01 2.19-.66 2.77-1.61.19-.33.4-.67.41-1.06.1-1.79.06-3.57.07-5.36.01-4.03-.01-8.05.02-12.07z"/></svg>
                ...
                {selectedVirtualCountry ? (
                  <div className="flex items-center space-x-2.5 sm:space-x-3 flex-1 min-w-0">
                    {selectedVirtualCountry === 'GLOBAL' ? (
                      ...
                      <div className="w-8 h-5 overflow-hidden flex items-center justify-center bg-gray-700 border border-gray-300 rounded-[2px] flex-shrink-0 sim-card-flag">
                        <img 
                  ...
                  >
                    <div className="flex items-center space-x-2.5 sm:space-x-3 min-w-0 flex-1">
                      ...
                      <div className="min-w-0 flex-1 text-left">
                        ...
                        {country.subtext && (
                          <span className="text-xs text-gray-500 dark:text-gray-400 block">{country.subtext}</span>
                        )}
                      </div>
                    </div>
                    {selectedVirtualCountry === country.code && (
          ...
          <div className="mb-3 sm:mb-4">
            ...
            {selectedVirtualCountry ? (
              <div className="space-y-1.5 sm:space-y-2">
                ...
                <div className="flex items-center space-x-1.5 sm:space-x-2 px-2 sm:px-3 py-1.5 sm:py-2 bg-green-50 dark:bg-green-900/20 rounded-lg mt-2 sm:mt-3">
                  <svg className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-green-600 dark:text-green-400 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
                  </svg>
                  <span className="text-[11px] sm:text-xs md:text-sm text-gray-700 dark:text-gray-300 font-medium whitespace-nowrap">
                    <span className="font-semibold">Keep forever:</span> Top-up anytime
                  </span>
                </div>
              </div>
            ) : (
